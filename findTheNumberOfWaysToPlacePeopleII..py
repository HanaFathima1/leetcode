...
class Solution:
    def numberOfPairs(self, points: List[int]) -> List[int]:
        points.sort(key = lambda p:(p[0], -p[1]))
        n = len(points)
        count = 0
        for i in range(n):
            point_A = points[i]   #point_A: coordinates of Alice.
            yMax = -float('inf')  #yMax: tracks the highest y among valid “Bobs” we’ve chosen so far.This ensures no other point lies inside the rectangle defined by A and B.
            for j in range(i+1, n):
                point_B = points[j]
                #check if B can be a lower-right corner for A
                #point_A[1] is y_A, point_B[1] is y_B
                if point_A[1] >= point_B[1]:
                    #we found a potential Bob. Now, is it valid?
                    #its valid if its y is higher thhan thhe last valid bob's y
                    #but still lower than Alice's y
                    #this check ensures no other point is contained
                    if point_B[1] > yMax:
                        count += 1
                        yMax = point_B[1]
        return count
