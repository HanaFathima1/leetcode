...
class Solution:
    def shufle(self, nums:list[int], n:int) -> list[int]:
        ans = []
        for i in range(n):
            ans.append(nums[i])
            ans.append(nums[i+n])
        return ans
sol = Solution()
print(sol.shufle(nums = [2,5,1,3,4,7], n = 3))
print(sol.shufle(ums = [1,2,3,4,4,3,2,1], n = 4))
print(sol.shufle(nums = [1,1,2,2], n = 2))